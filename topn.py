...
from dataloader import LoadDataset, LoadReviewData, process_template,preprocess_opt2, preprocess_opt, opt_pure_tokenize,preprocess_opt_preference, LoadDataset_preference
...
logging.basicConfig(level=logging.INFO, filename=f'../logs6/New_topn_experiment_{args.dataset}_{current_time}_{args.prefix}.log', filemode='w')
...
processed_data_path = f"{review_data.data_dir}/topn_reviews_{base_model_name}.pickle"
...
if not os.path.exists(processed_data_path):
    ...
    preprocess_opt2(review_data, base_model_name, tokenizer, bos, eos, pad_id)
    logging.info(now_time() +f"Processed data saved in {review_data.data_dir}/topn_reviews_{base_model_name}.pickle")
...
train_dataset = LoadDataset(data_dir, processed_data_path, args.index, "train")
...
def evaluate(dataloader):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            if model.stage==2 or model.stage==3:
